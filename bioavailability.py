   ...
   if option == 'Изучение абсолютной и относительной биодоступности препарата':
       ...
       with col1:
           ...
           if panel == "Загрузка файлов":
              ...
              uploaded_file_3 = st.file_uploader("Выбрать файл перорального введения таблетки (формат XLSX)", key='Файл перорального введения таблетки при изучении абсолютной и относительной биодоступности препарата')
              ...
              ############### Параметры ФК
                 ...
                 if "feature_disable_selected_value_tab" not in st.session_state:
                     st.session_state["feature_disable_selected_value_tab"] = True
                 ...
                 if selected_columns != [] and st.session_state["feature_disable_selected_value_tab"]:
                 ...
                 if len(list_cmax_1_tab) == len(df.index.tolist()):
                    ...
                    if "feature_disable_selected_value_2" not in st.session_state:
                     st.session_state["feature_disable_selected_value_tab_2"] = True
                    ...
                    if selected_columns_2 != [] and st.session_state["feature_disable_selected_value_tab_2"]:
              ...
              if ("uploaded_file_1" in st.session_state) and ("uploaded_file_2" in st.session_state) and ("uploaded_file_3" in st.session_state) and measure_unit and dose_iv and dose_po_sub and dose_po_tab:
                 ...
                 if condition_ON_cmax2 == False:
                    if (condition_iv_cmax1 and condition_sub_cmax1 and condition_tab_cmax1):
                       button_calculation_bioavailability = True
            ...
            if (list_keys_file_org != []) and dose and measure_unit_org_blood and measure_unit_org_organs:
                ...
                if (list_keys_file_org != []) and dose and measure_unit_org_blood and measure_unit_org_organs and button_calculation:
                   ...
                   for i in list_df_unrounded: 
                       ...
                       list_list_PK_par_mean.append([mean_сmax,mean_сmax2,mean_tmax,mean_mrt0inf,mean_thalf,mean_auc0t,mean_auc0inf,mean_aumc0inf,mean_kel])

                   ### получение итогового фрейма ФК параметров органов
                   df_PK_organs_total = pd.DataFrame(list_list_PK_par_mean, columns =['Cmax','Cmax(2)''Tmax','MRT0→∞','T1/2','AUC0-t','AUC0→∞','AUMC0-∞','Kel'],index=list_name_organs)
                   df_PK_organs_total_transpose=df_PK_organs_total.transpose()
                   
                   st.write(list_name_organs)
                   ...
                   df_PK_organs_total_transpose.index=['Cmax ' +"("+measure_unit_org_blood+")",'Cmax(2) ' +"("+measure_unit_org_blood+")",'Tmax ' +"("+"ч"+")",'MRT0→∞ '+"("+"ч"+")",'T1/2 '+"("+"ч"+")",'AUC0-t '+"("+measure_unit_org_blood+"×ч" +")",'AUC0→∞ '+"("+measure_unit_org_blood+"×ч" +")",'AUMC0-∞ '+"("+measure_unit_org_blood+"×ч\u00B2" +")",'Kel '+"("+"ч\u207B\u00B9"+")",'fт']
                   ...
                   df_total_total_organs = pd.concat([series_Cmax,series_Cmax2, series_Tmax, series_MRT0_inf,series_half_live,series_AUC0_t,series_AUC0_inf,series_AUMC0_inf,series_Kel,series_ft], axis= 1)
